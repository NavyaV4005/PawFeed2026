...
// ── Script block 3: recipeDB + main app logic ───────────────────────────────
    ...
    async function saveWeeklyPlan(plan) {
      ...
      try {
        const { error } = await window.supabaseClient.from('weekly_meal_plan').upsert({
          user_id: currentUser.id,
        ...
        if (error) {
          console.error("Error syncing weekly plan to Supabase:", error);
          showToast("Failed to save meal plan to cloud.");
        }
      } catch (err) {
    ...
    function autoGenerateWeeklyPlan() {
      ...
      pawCache.weeklyPlan = plan;
      renderWeeklyPlan();
      showToast("AI Meal Plan Generated! 🤖✨ (Click Save Plan!)");
    }
...
window.renderMedicalRecordsBox = function() {
    ...
    const records = (pawCache.medicalRecords || []).filter(r =>
        ...
        String(r.pet_id) === String(activeIdx) || 
        String(r.pet_id) === ('local_' + activeIdx) ||
        !r.pet_id // fallback to show if somehow missing
    );
